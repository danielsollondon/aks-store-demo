...
- `aks-project/pipeline-repo: danielsollondon/aks-store-demo`
...
- Trigger ONLY on `workflow_dispatch` with the following required string inputs (and their defaults):
  - `cluster-name` (default: `clu-10849`)
  - `resource-group` (default: `dansol19959`)
  - `namespace` (default: `aks-store-pipe`)
...
- Build and push the container image using ACR Tasks: `az acr build --registry ${{ secrets.AZURE_ACR_NAME }} --image aks-store-demo:${{ github.sha }} .`
- Update the container image reference in manifests to use `bbashreg19959.azurecr.io/aks-store-demo:${{ github.sha }}`
...
## Naming Conventions
- PR title: "[AKS Desktop] Add deployment pipeline for aks-store-demo"
- Commit messages: prefixed with "deploy:"
- K8s resource names: kebab-case, prefixed with `aks-store-demo`

## Validation Steps
- Run `kubectl apply --dry-run=client -f deploy/kubernetes/` to validate manifests
- Validate Dockerfile builds cleanly
- Ensure all manifests target namespace: `aks-store-pipe`
